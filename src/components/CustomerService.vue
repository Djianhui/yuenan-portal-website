    ...
    >
      ...
      <div class="bg-gradient-to-r from-cyan-400 to-blue-400 p-4 rounded-t-lg flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-white font-bold">{{ t('customerService.chatTitle') }}</span>
        </div>
        <div class="flex items-center gap-2">
      ...
      <div class="border-t p-3">
        ...
        <div class="text-xs text-gray-400 mt-2 flex items-center gap-1">
          ...
          <span>{{ t('customerService.notice') }}</span>
        </div>
      </div>
    </div>
  </div>
...
const customerImage = computed(() => {
  return locale.value === 'en-US' 
    ? new URL('../img/customer.png', import.meta.url).href
    : new URL('../img/customer.png', import.meta.url).href
})
...
const initialMessages = computed(() => {
  if (locale.value === 'en-US') {
    return [
      {
        sender: 'Changsha Yuean CS - Xiao Yue',
        time: '12-03 09:24:52',
        content: '<strong>[Changsha Yuean]</strong> is committed to one-stop comprehensive enterprise services. Our platform provides nationwide business registration, taxation, social security, intellectual property, and legal consultation. You can also call our free hotline <strong>400-872-8735</strong>.'
      },
      {
        sender: 'Changsha Yuean CS - Xiao Yue',
        time: '12-03 09:24:52',
        content: 'Hello, I am <strong class="text-orange-500">Customer Service - Xiao Yue</strong> from Changsha Yuean. How may I help you today?'
      },
      {
        sender: 'Changsha Yuean CS - Xiao Yue',
        time: '12-03 09:26:09',
        content: 'Is there anything I can assist you with?'
      }
    ]
  }
  return [
...
const sendMessage = () => {
  if (inputMessage.value.trim()) {
    messages.value.push({
      sender: locale.value === 'en-US' ? 'Me' : '我',
      time: new Date().toLocaleString(locale.value === 'en-US' ? 'en-US' : 'zh-CN', { 
        month: '2-digit', 
        day: '2-digit', 
        hour: '2-digit', 
        minute: '2-digit', 
    ...
    setTimeout(() => {
      messages.value.push({
        sender: locale.value === 'en-US' ? 'Changsha Yuean CS - Xiao Yue' : '长沙悦安客服-小悦',
        time: new Date().toLocaleString(locale.value === 'en-US' ? 'en-US' : 'zh-CN', { 
          month: '2-digit', 
          day: '2-digit', 
          hour: '2-digit', 
        ...
        content: locale.value === 'en-US' 
          ? 'Thank you for your inquiry. Our customer service team will respond to you shortly. You can also call our hotline <strong>400-872-8735</strong> for immediate assistance.'
          : '感谢您的咨询，我们的客服人员会尽快为您解答。您也可以拨打热线 <strong>400-872-8735</strong> 获取即时帮助。'
      })
    }, 1000)
  }
}
